...
const formatPassengerName = (passenger) => {
  if (!passenger) return 'PASSENGER NAME NOT PROVIDED';
  ...
  const prefix = passenger.prefix ? prefixMap[passenger.prefix.toLowerCase()] || '' : '';
  
  let fullName = '';
  if (prefix) fullName += `${prefix} `;
  if (passenger.firstName) fullName += `${passenger.firstName} `;
  if (passenger.middleName) fullName += `${passenger.middleName} `;
  if (passenger.lastName) fullName += passenger.lastName;
  
  return fullName.trim() || 'PASSENGER NAME NOT PROVIDED';
};
...
const renderHeader = (doc, data) => {
  ...
  doc.fillColor('#FFFFFF')
     .fontSize(24)
     .font('Helvetica-Bold')
     .text(`${data.airline?.toUpperCase() || 'AIRLINE'}`, LAYOUT.MARGIN, 20);
...
const renderTicketInfo = (doc, data, pnr, ticketNumber) => {
  ...
  // Get primary passenger name for the header
  let primaryPassengerName = '';
  if (data.passengers && data.passengers.length > 0) {
    primaryPassengerName = formatPassengerName(data.passengers[0]);
  } else {
    // Fallback to old format
    primaryPassengerName = formatPassengerName({
      prefix: data.customerPrefix,
      firstName: data.customerFirstName,
      middleName: data.customerMiddleName,
      lastName: data.customerLastName
    }) || data.customerName || 'PASSENGER NAME';
  }
  ...
  doc.fillColor('#000000')
     .font('Helvetica')
     .fontSize(11)
     .text(primaryPassengerName.toUpperCase(), 
           LAYOUT.MARGIN + 10, 
           boxY + 25, 
           {
             width: 180,
...
// ✅ FIXED: Show ALL passengers from the passengers array
const renderPassengerDetails = (doc, data) => {
  const sectionTitleY = doc.y;
  ...
  // Calculate required height based on number of passengers
  const passengerCount = passengers.length || 1;
  const baseHeight = 120;
  const additionalPerPassenger = 50; // Extra height per additional passenger
  const boxHeight = passengerCount > 1 
    ? baseHeight + ((passengerCount - 1) * additionalPerPassenger)
    : baseHeight;
  ...
  doc.fillColor('#374151')
     .fontSize(10)
     .font('Helvetica-Bold')
     .text('PASSENGERS & PERSONAL DETAILS', LAYOUT.MARGIN + 10, boxY + 10)
     .text('CONTACT INFORMATION', LAYOUT.MARGIN + 210, boxY + 10)
     .text('FARE TYPE', LAYOUT.MARGIN + 410, boxY + 10);
  ...
  if (passengers && passengers.length > 0) {
    // Show all passengers from the array
    passengers.forEach((passenger, index) => {
      const passengerName = formatPassengerName(passenger);
      
      // Passenger name
      doc.fillColor('#000000')
         .font('Helvetica')
         .fontSize(11)
         .text(`${index + 1}. ${passengerName.toUpperCase()}`, 
               LAYOUT.MARGIN + 10, 
               currentY, 
               {
                 width: 180,
      ...
      let personalDetails = [];
      if (passenger.dob) personalDetails.push(`DOB: ${passenger.dob}`);
      if (passenger.gender) personalDetails.push(`Gender: ${passenger.gender.charAt(0).toUpperCase() + passenger.gender.slice(1)}`);
      ...
      currentY += 35; // Space for next passenger
    });
  } else {
    // Fallback to single passenger from old fields
    const passengerName = formatPassengerName({
      prefix: data.customerPrefix,
      firstName: data.customerFirstName,
      middleName: data.customerMiddleName,
      lastName: data.customerLastName
    }) || data.customerName || 'PASSENGER NAME NOT PROVIDED';
    
    doc.fillColor('#000000')
       .font('Helvetica')
       .fontSize(11)
       .text(`1. ${passengerName.toUpperCase()}`, 
             LAYOUT.MARGIN + 10, 
             currentY, 
             { width: 180 });
    
    let personalDetails = [];
    if (data.customerDOB) personalDetails.push(`DOB: ${data.customerDOB}`);
    if (data.customerGender) personalDetails.push(`Gender: ${data.customerGender.charAt(0).toUpperCase() + data.customerGender.slice(1)}`);
  ...
  // Contact information - fixed position (doesn't change with passenger count)
  const contactY = boxY + 25;
  doc.fillColor('#000000')
     .fontSize(10)
     .text(`Email:`, LAYOUT.MARGIN + 210, contactY);
...
doc.y = sectionTitleY;
  ...
  doc.fillColor('#000000')
     ...
     .text(data.flightNumber || `${data.airlineCode || 'AA'} ${Math.floor(Math.random() * 9000 + 1000)}`, 
           LAYOUT.MARGIN + 10, flightRowY)
     .text(data.departure?.toUpperCase() || 'N/A', LAYOUT.MARGIN + 100, flightRowY)
     .text(data.arrival?.toUpperCase() || 'N/A', LAYOUT.MARGIN + 250, flightRowY)
     .text(duration, LAYOUT.MARGIN + 350, flightRowY)
     .text(data.cabinClass || 'ECONOMY', LAYOUT.MARGIN + 430, flightRowY);
  ...
  doc.fillColor('#6B7280')
     .fontSize(9)
     .font('Helvetica-Oblique')
     .text(`Date: ${data.travelDate || 'N/A'}`, LAYOUT.MARGIN + 100, flightRowY + 17)
     .text(`Time: ${data.departureTime || 'To be announced'}`, LAYOUT.MARGIN + 100, flightRowY + 32)
     .text(`Terminal: ${data.departureTerminal || 'To be announced'}`, LAYOUT.MARGIN + 100, flightRowY + 47)
     .text(`Gate: To be announced`, LAYOUT.MARGIN + 100, flightRowY + 62);
  
  // Arrival details
  doc.text(`Date: ${data.travelDate || 'N/A'}`, LAYOUT.MARGIN + 250, flightRowY + 17)
     .text(`Time: ${data.arrivalTime || 'To be announced'}`, LAYOUT.MARGIN + 250, flightRowY + 32)
     .text(`Terminal: ${data.arrivalTerminal || 'To be announced'}`, LAYOUT.MARGIN + 250, flightRowY + 47)
     .text(`Seat: To be assigned at check-in`, LAYOUT.MARGIN + 250, flightRowY + 62);
...
const renderImportantInfo = (doc, data) => {
  ...
  doc.fillColor('#000000')
     ...
     .font('Helvetica')
     .list([
       `Check-in opens 24 hours before departure and you can do the checkin on the airlines website (${data.airline || 'airline'})/mobile app`,
       `Boarding begins 2 hours before departure`,
       `Government-issued photo ID required for all passengers`,
       `Electronic ticket - No physical ticket required`,
       `Changes/Cancellations: Fees apply. Visit airline website for details`
...
const renderPaymentSummary = (doc, data) => {
  ...
  doc.fillColor('#1F2937')
     .fontSize(14)
     .font('Helvetica-Bold')
     .text('PAYMENT SUMMARY', LAYOUT.MARGIN + 10, boxY + 18);
  
  // Column headers
  doc.fillColor('#4B5563')
     .fontSize(10)
     .font('Helvetica-Bold')
     .text('DESCRIPTION', LAYOUT.MARGIN + 10, boxY + 40)
     .text('AMOUNT (USD)', LAYOUT.MARGIN + 360, boxY + 40);
  
  // Get passenger count for display
  ...
  doc.fillColor('#000000')
     .font('Helvetica')
     .fontSize(10)
     .text(`Base Fare (${passengerCount} Passenger${passengerCount > 1 ? 's' : ''})`, LAYOUT.MARGIN + 10, boxY + 55)
     .text(`$${Number(data.bookingAmount * 0.7).toFixed(2)}`, LAYOUT.MARGIN + 360, boxY + 55)
     .text('Taxes & Fees', LAYOUT.MARGIN + 10, boxY + 70)
     .text(`$${Number(data.bookingAmount * 0.3).toFixed(2)}`, LAYOUT.MARGIN + 360, boxY + 70);
  ...
  return boxY + boxHeight + LAYOUT.SECTION_SPACING + 10;
};
...
// ✅ FIXED: Main export function that properly handles passengers array
export const generateETicket = async (data) => {
  console.log("🎫 E-Ticket Generation Data:", {
    confirmationNumber: data.confirmationNumber,
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      renderPassengerDetails(doc, data); // ✅ Now shows ALL passengers
      doc.moveDown(0.5);
      ...
      stream.on("finish", () => {
        console.log(`✅ E-Ticket generated with ${data.passengers?.length || 1} passenger(s)`);
        resolve(filePath);
      });
